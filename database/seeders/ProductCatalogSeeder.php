...
class ProductCatalogSeeder extends Seeder
{
    public function run(): void
    {
        ...
        ])->mapWithKeys(fn ($name) => [$name => ProductGroup::firstOrCreate(['name' => $name])]);

        $types = collect([
            'Laptop', 'Desktop', 'Switch', 'Router', 'Monitor',
            'Access Point', 'Server', 'Printer', 'UPS', 'NAS',
        ])->mapWithKeys(fn ($name) => [$name => ProductType::firstOrCreate(['name' => $name])]);
        ...
        foreach ($brandsData as $brandName => $modelNames) {
            ...
            foreach ($modelNames as $modelName) {
                $model = ProductModel::firstOrCreate(['product_brand_id' => $brand->id, 'name' => $modelName]);
