...
### P1 - High
- [ ] Scope admins can change the global catalog through the Select `createOptionForm`/`editOptionForm` (gated only by `isAdmin()`, policies never checked): Product in `InventoryResource`, Brand/Model in `ProductResource`, TaskType/TaskStatus in `TaskResource`; same pattern for InventoryPosition in `MovementResource` and InventoryLocation in `InventoryPositionResource` (no check at all)
- [ ] Stock recalculation race on MySQL/MariaDB: `Stock::updateStockByMovementItems()` sums without locking the stock row and writes an absolute value (only the origin stock of withdrawals is locked); concurrent loads/deletions lose updates or bypass the non-negative check
